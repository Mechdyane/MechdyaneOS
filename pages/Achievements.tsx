    ...
    <div className="p-8 md:p-12 space-y-12 h-full bg-[#020617]/40 overflow-y-auto custom-scrollbar">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-12">
        <div className="space-y-4">
          ...
          <p className="text-slate-500 font-medium text-lg max-w-2xl">Visualization of your neural milestones and growth markers.</p>
        </div>
        
        <div className="flex items-center gap-6 group">
           <div className="text-right flex flex-col justify-center">
             <p className="text-[10px] font-black text-blue-500/60 uppercase tracking-[0.4em] mb-1 font-orbitron">Completion</p>
             <p className="text-4xl font-black text-blue-400 font-orbitron leading-none">{completionPercentage}%</p>
           </div>
           <div className="w-16 h-16 rounded-[1.5rem] bg-blue-600/10 border border-blue-500/20 flex items-center justify-center shadow-[0_0_30px_rgba(59,130,246,0.1)] relative">
              <div className="absolute inset-0 border border-blue-500/30 rounded-[1.5rem] rotate-45 scale-90 group-hover:rotate-90 transition-transform duration-700"></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {achievements.map(a => {
          ...
          const rarityClass = rarityColors[a.rarity as keyof typeof rarityColors] || rarityColors.Common;
          
          return (
            <div 
              key={a.id} 
              className={`relative p-8 md:p-10 rounded-[3rem] border transition-all duration-500 flex items-center gap-10 group overflow-hidden
                ${isUnlocked 
            ...
            >
              ...
              <div className={`w-24 h-24 rounded-[2rem] flex items-center justify-center shrink-0 transition-all duration-700 relative z-10 
                ${isUnlocked ? 'bg-white/[0.03] border-2 ' + rarityClass.split(' ')[1] : 'bg-slate-900/80 border border-slate-800 text-slate-700'}
                group-hover:scale-105 group-hover:rotate-2
              `}>
                <i className={`fas ${a.icon} text-4xl ${isUnlocked ? 'text-white' : 'text-slate-800'}`}></i>
                
                {/* Unlock Glow */}
                {isUnlocked && (
                  <div className={`absolute inset-0 rounded-[2rem] blur-xl opacity-20 bg-current transition-opacity group-hover:opacity-40`}></div>
                )}
              </div>
              ...
              <div className="flex-1 space-y-4 relative z-10">
                <div className="flex justify-between items-center pr-12">
                  ...
                  {isUnlocked && (
                    <div className="w-8 h-8 rounded-full bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center text-emerald-400 animate-in zoom-in duration-500">
                ...
                {!isUnlocked && (
                  <div className="pt-6 space-y-3">
                     <div className="flex justify-between items-end">
                        <span className="text-[9px] font-black uppercase tracking-[0.3em] text-slate-700 font-orbitron leading-none">Neural Link Progress</span>
                        <span className="text-[10px] font-black text-slate-600 font-orbitron leading-none">{a.progress} / {a.target}</span>
                     </div>
              ...
              <div className="absolute top-0 right-0 p-12 opacity-[0.02] pointer-events-none group-hover:opacity-[0.05] transition-opacity">
                 <i className={`fas ${a.icon} text-[180px]`}></i>
              </div>
            </div>
          );
        })}
      ...
      <footer className="pt-24 text-center opacity-20">
         <p className="text-[10px] font-black text-slate-500 uppercase tracking-[1em] font-orbitron">Primary Operational Node</p>
      </footer>
    </div>
  );
};
